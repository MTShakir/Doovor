...
export async function getAccessContext(client: DbClient, userId: string): Promise<AccessContext> {
  const [staff, memberships, learner, instructors] = await Promise.all([
    ...
    client.from('instructor_profiles').select('id, business_id').eq('user_id', userId),
  ]);
  ...
  const profileByBusiness = new Map((instructors.data ?? []).map((p) => [p.business_id, p.id]));
  ...
  return {
    ...
    memberships: (memberships.data ?? [])
      ...
      .map((m) => ({
        ...
        role: m.role,
        instructorProfileId: profileByBusiness.get(m.business_id) ?? null,
      })),
  };
}
