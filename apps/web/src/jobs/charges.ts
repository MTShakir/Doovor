...
type FailedBecause = 'no_card' | 'expired_card' | 'declined' | 'authentication_required';
...
export async function chargeBeforeLessons(options: { withinHours?: number; now?: Date } = {}): Promise<ChargeResult> {
  ...
  const fail = async (bookingId: string, reason: FailedBecause): Promise<void> => {
    const marked = await supabase.rpc('system_record_charge_failed', { p_booking_id: bookingId, p_reason: reason });
  ...
  for (const one of due) {
    if (one.customer_id === null) {
      await fail(one.booking_id, 'no_card');
      continue;
    }

    const listed = await provider.listSavedCards({ accountId: one.account_id, customerId: one.customer_id });
    if (!listed.ok) {
    ...
    const [card] = usableCards(listed.data, now);
    if (!card) {
      await fail(one.booking_id, listed.data.length > 0 ? 'expired_card' : 'no_card');
      continue;
    }
    ...
    if (!charged.ok) {
      // A provider that cannot be reached is tried again on the next run: nothing was decided.
    ...
    if (charged.data.status === 'requires_action' || charged.data.status === 'requires_payment_method') {
      await fail(one.booking_id, 'authentication_required');
      continue;
    }
