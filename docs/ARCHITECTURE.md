...
- A card typed in goes into the Payment Element (`apps/web/src/components/payments/card-form.tsx`). It loads Stripe.js for the connected account (`stripeAccount`), draws the fields for the payment's or set-up's client secret, and confirms in the browser; the webhook records what happened. The error words come from `apps/web/src/lib/payments/card-errors.ts`. When a kept card needs the bank's check, the charge answers with its client secret and the screen runs `handleNextAction` (D-099). The Permissions-Policy header lets Stripe's frame use the Payment Request API, and the report-only CSP lists Stripe's script and frame origins.
